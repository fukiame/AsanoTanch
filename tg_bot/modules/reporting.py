...
@kigcmd(command='reports', block=False)
@spamcheck
@bot_admin_check()
...
@kigcmd(command='report', filters=filters.ChatType.GROUPS, group=REPORT_GROUP, block=False)
@kigmsg((filters.Regex(r"(?i)@admin(s)?")), group=REPORT_GROUP, block=False)
@spamcheck
@user_not_admin_check
@loggable
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    splitter = await query.data.replace("reported_", "").split("=")
    if splitter[1] == "kick":
        try:
            ...
            await query.answer(f"🛑 Failed to kick\n{err}")
        ...
        try:
            ...
            #     parse_mode=ParseMode.HTML,
            # )
            await query.answer(f"🛑 Failed to Ban\n{err}", show_alert=True)
