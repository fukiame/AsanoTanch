...
from .. import BACKUP_PASS, CASH_API_KEY, CF_API_KEY, DB_URI, LASTFM_API_KEY, TOKEN, dispatcher
...
def logs(update: Update, context: CallbackContext):
    ...
    with open(logsname, "wt") as logsout:
        for line in logstxt:
            logsout.write(line.replace(str(DB_URI), '$DATABASE').replace(str(TOKEN), '$TOKEN').replace(str(CASH_API_KEY), '$CASH_API_KEY').replace(str(LASTFM_API_KEY), '$LASTFM_API_KEY').replace(str(CF_API_KEY), '$CF_API_KEY').replace(str(BACKUP_PASS), '$BACKUP_PASS'))
        logstxt.close()
...
def updates_log(update: Update, context: CallbackContext):
    ...
    with open(updatesname, "wt") as updatesout:
        for line in updatestxt:
            updatesout.write(line.replace(str(DB_URI), '$DATABASE').replace(str(TOKEN), '$TOKEN').replace(str(CASH_API_KEY), '$CASH_API_KEY').replace(str(LASTFM_API_KEY), '$LASTFM_API_KEY').replace(str(CF_API_KEY), '$CF_API_KEY').replace(str(BACKUP_PASS), '$BACKUP_PASS'))
        updatestxt.close()
