...
def logs(update: Update, context: CallbackContext):
    ...
    with open(logsname, "wt") as logsout:
        for line in logstxt:
            logsout.write(line.replace(str(DB_URI), '$DATABASE').replace(str(TOKEN), '$TOKEN').replace(str(CASH_API_KEY), '$CASH_API_KEY').replace(str(TIME_API_KEY), '$TIME_API_KEY').replace(str(LASTFM_API_KEY), '$LASTFM_API_KEY').replace(str(CF_API_KEY), '$CF_API_KEY').replace(str(spamwatch_api), '$spamwatch_api').replace(str(BACKUP_PASS), '$BACKUP_PASS'))
        logstxt.close()
